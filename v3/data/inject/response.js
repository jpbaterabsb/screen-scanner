  ...
  const run = em.run = async (oResult, mode = 'normal') => {
    ...
    try {
      ...
      if (o.text.trim() === '') {
        ...
        let data;
        try {
          data = await fetch('https://generativelanguage.googleapis.com/v1beta2/models/text-bison-001:generateText?key=' + apiKey, requestOptions)
        ...
        command('buildAI', data.candidates[0].output);
        command('enable');
      }
    }
    catch (e) {
